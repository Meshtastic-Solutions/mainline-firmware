...
"""Generate markdown and JSON inventories of board-support metadata and common target-definition macros.

The markdown output (docs/hardware-support-context.md) is the human/agent view.
The JSON output (docs/hardware-support-context.json) is the machine view consumed
by bin/board_intake.py and external tooling such as web dashboards.
"""
...
JSON_OUTPUT_PATH = ROOT / "docs" / "hardware-support-context.json"

# Non-custom_meshtastic env keys worth inventorying: CI build tier, PIO board id,
# compile-check marker, and base-environment lineage.
ENV_SETTINGS_KEYS = [
    "board",
    "board_level",
    "board_check",
...
def parse_platformio_envs(path: Path) -> list[dict[str, object]]:
    ...
    for raw_line in path.read_text(encoding="utf-8").splitlines():
        ...
        if line.startswith("[") and line.endswith("]"):
            if current:
                envs.append(current)
            current = {"section": line[1:-1], "metadata": {}, "settings": {}}
            continue
...
def markdown_table(header: list[str], rows: list[list[str]]) -> list[str]:
    """Render a column-padded markdown table matching prettier's output style.

    Emitting prettier-stable tables directly means regenerating the doc never
    requires a `trunk fmt` pass to keep CI formatting checks green.
    """
    widths = [
        max(3, len(head), *(len(row[i]) for row in rows)) if rows else max(3, len(head))
    ...
    lines = [fmt_row(header), fmt_row(["-" * width for width in widths])]
...
def collect_inventory() -> dict[str, object]:
    ...
    total_envs = 0
    no_variant_h: defaultdict[str, int] = defaultdict(
        int
    )  # arch -> count of dirs with no variant.h
    ...
    for platformio_path in sorted(VARIANTS_DIR.glob("**/platformio.ini")):
        ...
        # Use the first path segment under variants/ so nested layouts like
        # variants/<arch>/diy/<board> attribute to <arch>, not "diy".
        rel_parts = variant_dir.relative_to(VARIANTS_DIR).parts
        board_level_arch_raw = rel_parts[0] if len(rel_parts) >= 2 else "unknown"
        ...
        for env in envs:
            ...
            if metadata:
                has_metadata += 1
            architecture = str(
                metadata.get("custom_meshtastic_architecture") or board_level_arch
            )
            ...
            entry = {
                ...
                "metadata": metadata,
                "settings": dict(env.get("settings", {})),
                "defines": defines,
...
def render_markdown(inventory: dict[str, object]) -> str:
    ...
    for category in [
        ...
        lines.append("")
        rows = [
            [f"`{name}`", f"{count} variants", MACRO_DESCRIPTIONS.get(name, "")]
            for name, count in counter.most_common(15)
        ]
        lines.extend(markdown_table(["Macro", "Used in", "Description"], rows))
        lines.append("")
    ...
    for architecture in sorted(architectures):
        entries = sorted(
            architectures[architecture], key=lambda item: item["environment"]
        )
        ...
        for entry in entries:
            ...
            rows.append(
                [
                    ...
                    metadata.get("custom_meshtastic_display_name", ""),
                    metadata.get("custom_meshtastic_hw_model", ""),
                    metadata.get("custom_meshtastic_hw_model_slug", ""),
    ...
    for architecture in sorted(architectures):
        ...
        for category in [
            ...
            selected = [name for name in defines if classify_macro(name) == category][
                :8
            ]
            if not selected:
                continue
    ...
    if no_variant_h:
        lines.append(
            ...
            + ", ".join(
                f"{arch}: {count}" for arch, count in sorted(no_variant_h.items())
            )
        )
        lines.append("")
...
def render_json(inventory: dict[str, object]) -> str:
    """Render the inventory as deterministic JSON for machine consumers.

    Consumed by bin/board_intake.py (pattern matching) and external tooling
    (e.g. web dashboards). Keys are sorted and the variant walk is already
    deterministic, so regenerating without repository changes is a no-op diff.
    """
    architectures = inventory["architectures"]
    arch_obj: dict[str, list[dict[str, object]]] = {}
    for architecture in sorted(architectures):
        entries = sorted(
            architectures[architecture], key=lambda item: item["environment"]
        )
        arch_obj[architecture] = [
            {
                ...
                "settings": dict(sorted(entry.get("settings", {}).items())),
                "defines": dict(sorted(entry["defines"].items())),
                "category_counts": dict(sorted(entry["category_counts"].items())),
            }
            for entry in entries
        ]
    ...
    frequency_obj = {
        ...
        for category, counter in sorted(category_frequency.items())
    }

    document = {
        "$comment": "Generated by bin/generate_hardware_support_context.py — do not edit by hand.",
        "scope": {
            "variant_dirs_scanned": inventory["total_variant_dirs"],
            "environments_summarized": inventory["total_envs"],
            "environments_with_metadata": inventory["has_metadata"],
        },
        "metadata_keys": PLATFORMIO_METADATA_KEYS,
        "settings_keys": ENV_SETTINGS_KEYS,
        "architectures": arch_obj,
        "category_frequency": frequency_obj,
        "variant_dirs_without_variant_h": dict(
            sorted(inventory.get("no_variant_h", {}).items())
        ),
    }
...
def print_validate_summary(inventory: dict[str, object]) -> None:
    ...
    for family in _BSP_DEFAULT_FAMILIES:
        marker = (
            "YES"
            if family in {a.split("-")[0] for a in architectures}
            else "not in scan"
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate hardware support context markdown and JSON"
    )
    parser.add_argument(
        "--output", default=str(OUTPUT_PATH), help="Output markdown path"
    )
    parser.add_argument(
        "--json-output",
        default=str(JSON_OUTPUT_PATH),
        help="Output JSON path (machine-readable inventory)",
    )
    parser.add_argument(
        "--validate",
        action="store_true",
